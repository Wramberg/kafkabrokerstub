...
private:
	...
	class dummy : public kbs::kafka_elementI { };
	void exception_tests()
	{
		dummy testDummy;

		bool caught_exception(false);
		try { testDummy.deserialize(NULL); }
		catch(const std::runtime_error&) { caught_exception = true; }
		ASSERT_EQ(caught_exception, static_cast<bool>(true));

		caught_exception = false;
		try { testDummy.serialize(NULL); }
		catch(const std::runtime_error&) {caught_exception = true; }
		ASSERT_EQ(caught_exception, static_cast<bool>(true));

		caught_exception = false;
		try { testDummy.serial_size(); }
		catch(const std::runtime_error&) {caught_exception = true; }
		ASSERT_EQ(caught_exception, static_cast<bool>(true));
	}
